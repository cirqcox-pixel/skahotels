...
$propertyBranch = 'munyonyo';
$branch = ska_branch('munyonyo');
$munyonyoDining = cms_block('munyonyo', 'dining');
$munyonyoGarden = cms_block('munyonyo', 'garden');
$munyonyoVideo  = cms_block('munyonyo', 'hero_video');
$pageMeta = [
    'title'       => 'SKA Munyonyo | Lakeside Boutique Hotel Kampala',
    'description' => 'Book SKA Munyonyo — a serene boutique escape in Munyonyo, Kampala near Lake Victoria. Direct booking, free breakfast, seasonal rates and warm hospitality.',
    'path'        => 'munyonyo',
    'type'        => 'hotel',
    'image'       => $branch['image'],
    'schema'      => [
        ...
        '@type'     => 'Hotel',
        'name'      => $branch['full'],
        'url'       => ska_url('munyonyo'),
        'telephone' => $branch['phoneHref'],
        'email'     => $branch['email'],
        'address'   => ['@type' => 'PostalAddress', 'addressLocality' => 'Munyonyo, Kampala', 'addressCountry' => 'UG'],
...
<div class="pib-wrap">
  <div class="pib-container">
    ...
    <!-- LEFT: Name + Rating -->
    <div class="pib-left">
      <h1 class="pib-name">SKA The Boutique Munyonyo</h1>

      <div class="pib-rating">
        <!-- 5 filled / half / empty dot circles -->
        <div class="pib-dots" aria-label="Rating: 4.6 out of 5">
        ...
        <span class="pib-score">4.6</span>
        <span class="pib-bull">&bull;</span>
        <a href="https://maps.app.goo.gl/pZi16YiwXVtfhh4V9"
           target="_blank" rel="noopener"
           class="pib-reviews">12 Reviews</a>
      </div>
    </div>
...
<header class="hero">
  <video autoplay muted loop playsinline>
    <source src="<?= htmlspecialchars($munyonyoVideo['image'] ?: 'assets/video/ska_munyonyo.mp4') ?>" type="video/mp4">
  </video>
  <div class="overlay"></div>
  <div class="hero-content">
    <span class="hero-eyebrow">WELCOME TO SKA THE BOUTIQUE B&B — Munyonyo</span>
    <h1>Your serene boutique escape tucked away from the bustle of the city.</h1>
...
<section class="intro-banner">
  <div class="container text-center">
    ...
    <h2 class="intro-heading">Your serene boutique escape tucked away from the bustle of the city.</h2>
    <p class="intro-body">Whether you're in Kampala for business or leisure, SKA Munyonyo offers a warm and invigorating escape.
...
<section class="marketplace-section">
  <div class="marketplace-inner">
    <div class="marketplace-text">
      <h2>SKA Munyonyo</h2>
      <p>Experience refined comfort and warm hospitality at SKA The Boutique in Munyonyo, Uganda.
    ...
    <div class="marketplace-image">
      <img src="assets/images/ska_munyonyo_home.jpg" alt="Ska Munyonyo"
    ...
    <div class="more-ways-grid">
      <div class="mw-card">
        <div class="mw-img-side">
          <img src="<?= htmlspecialchars($munyonyoDining['image'] ?: 'assets/images/munyonyo/ska_about.jpg') ?>" alt="<?= htmlspecialchars($munyonyoDining['title'] ?: 'Restaurant') ?>"
               onerror="this.parentElement.style.background='#c8c4bc';this.remove()">
        </div>
        <div class="mw-text-side">
          <?php if ($munyonyoDining['tag']): ?><span class="mw-eyebrow"><?= htmlspecialchars($munyonyoDining['tag']) ?></span><?php endif; ?>
          <h3 class="mw-title"><?= htmlspecialchars($munyonyoDining['title'] ?: 'Fine Dining') ?></h3>
          <p class="mw-body"><?= htmlspecialchars($munyonyoDining['body'] ?: '') ?></p>
          <?php if ($munyonyoDining['link_url']): ?><a href="<?= htmlspecialchars($munyonyoDining['link_url']) ?>" class="mw-link"><?= htmlspecialchars($munyonyoDining['link_label'] ?: 'Learn More') ?> →</a><?php endif; ?>
        </div>
      </div>
      <div class="mw-card mw-card-reverse">
        <div class="mw-text-side">
          <?php if ($munyonyoGarden['tag']): ?><span class="mw-eyebrow"><?= htmlspecialchars($munyonyoGarden['tag']) ?></span><?php endif; ?>
          <h3 class="mw-title"><?= htmlspecialchars($munyonyoGarden['title'] ?: 'Serene Settings') ?></h3>
          <p class="mw-body"><?= htmlspecialchars($munyonyoGarden['body'] ?: '') ?></p>
          <?php if ($munyonyoGarden['link_url']): ?><a href="<?= htmlspecialchars($munyonyoGarden['link_url']) ?>" class="mw-link"><?= htmlspecialchars($munyonyoGarden['link_label'] ?: 'Learn More') ?> →</a><?php endif; ?>
        </div>
        <div class="mw-img-side">
          <img src="<?= htmlspecialchars($munyonyoGarden['image'] ?: 'assets/images/munyonyo/Armenities-4.jpg') ?>" alt="<?= htmlspecialchars($munyonyoGarden['title'] ?: 'Gardens') ?>"
...
<?php $galleryBranch = 'Munyonyo'; include 'includes/gallery-section.php'; ?>
...
<section class="section booking-section" id="book">
  <div class="container">
    ...
    <form method="POST" action="forms/process_contact_munyonyo.php"
          class="booking-form" id="bookingForm" data-ska-form="booking">
      <input type="hidden" name="branch" value="Munyonyo">
      <div class="row g-3">
...
/* ══════════════════════════════════════
   AUTO-FILL BOOKING FORM FROM URL PARAMS
══════════════════════════════════════ */
(function () {
  ...
  /* Fill inputs if values exist */
  if (checkin && document.getElementById('checkin')) {
    document.getElementById('checkin').value = checkin;
  }

  if (checkout && document.getElementById('checkout')) {
    document.getElementById('checkout').value = checkout;
  }

  /* Trigger price recalculation if dates exist */
  if (checkin && checkout && typeof calcFormTotal === 'function') {
    calcFormTotal();
  }
})();
...
/* ══════════════════════════════════════
   ...
   Called by Confirm & Book and by
   date/room changes in the form itself.
══════════════════════════════════════ */
function calcFormTotal() {
  const ci  = document.getElementById('checkin')?.value;
  const co  = document.getElementById('checkout')?.value;
  const p   = parseFloat(document.getElementById('formPrice')?.value) || 0;
  const el  = document.getElementById('totalPrice');
  if (!el) return;
  if (ci && co && p > 0) {
    const n = Math.max(0, (new Date(co) - new Date(ci)) / 86400000);
    el.innerHTML = `Total: <strong>USD ${(n * p).toLocaleString()}</strong>`;
  } else {
    el.innerHTML = 'Total: <strong>USD 0</strong>';
  }
}

/* Update when the user changes dates or selects room manually */
...
<!-- ══════════════════════════════════════════
     GETTING HERE
══════════════════════════════════════════ -->
<section class="getting-here-section" id="contact" data-ska-location="Munyonyo">
  <div class="container">
    <div class="getting-here-inner">
      <div class="gh-text">
        <span class="gh-eyebrow">OUR LOCATION</span>
        <h2 class="gh-title">GETTING HERE</h2>
        <address class="gh-address" data-ska-field="address">SKA The Boutique B&B — Munyonyo<br>Wavamunno, Kampala, Uganda</address>
        <p class="gh-phone" data-ska-field="phone"><i class="fa-solid fa-phone"></i> +256 200 98777</p>
        <p class="gh-email" data-ska-field="email"><i class="fa-solid fa-envelope"></i><a href="mailto:munyonyo.booking@skaboutiquebnb.com">munyonyo.booking@skaboutiquebnb.com</a></p>
        <p class="gh-email" data-ska-field="email_alt"><i class="fa-solid fa-envelope"></i><a href="mailto:skaboutiquebb@gmail.com">skaboutiquebb@gmail.com</a></p>
      ...
      <div class="gh-map">
        <iframe data-ska-field="map" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3989.781404895553!2d32.625248074723295!3d0.24647079975110975!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x177d9575a8b310b5%3A0xe6a89a69212b594d!2sSKA%20the%20Boutique%20B%26B!5e0!3m2!1sen!2sug!4v1774622522650!5m2!1sen!2sug"
