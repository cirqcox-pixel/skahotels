<?php
/**
 * FILE: forms/process_contact_naguru.php
 * Handles the Naguru booking form submission:
 *   1. Validates input
 *   2. Saves booking to the `bookings` table (status = 'pending')
 *   3. Emails the guest a "request received" confirmation
 *   4. Emails the admin team a "new booking" notification
...
$branch   = 'Naguru';                          // hard-coded for this form
...
if (!empty($errors)) {
    ...
    header("Location: ../naguru.php?error={$msg}#book");
    exit;
}
...
$total = $price * $nights;   // e.g. 150.00 * 3 = 450.00

/* ══════════════════════════════════════════════════
   4. SAVE TO DATABASE
   Columns: name, email, phone, whatsapp, room_type,
            price, checkin, checkout, total, message,
            season, branch, status, created_at
══════════════════════════════════════════════════ */
$dbSaved   = false;
$bookingId = null;
...
/* ══════════════════════════════════════════════════
   ...
   b) Admins — "new booking to review"
══════════════════════════════════════════════════ */
...
if ($dbSaved) {
    ...
    $adminSent = $mailer->sendAdminNewBooking($booking);
}
...
══════════════════════════════════════════════════ */
if ($dbSaved) {
    // 'success' = DB saved + guest email sent
    // 'saved'   = DB saved but email had an issue (booking is still safe)
    $flag = ($guestSent) ? 'success' : 'saved';
    header("Location: ../naguru.php?booking={$flag}#book");
} else {
    // Nothing was saved — surface a user-friendly error
    $err = urlencode('We could not record your booking. Please call us directly on +256 741 186 891.');
    header("Location: ../naguru.php?error={$err}#book");
}
exit;